    ...
    public class DistributorController : Controller
    {
        ...
        public async Task<ActionResult> AvailableProducts()
        {
            try
            {
                ...
                HttpResponseMessage response = await GlobalVariables.WebApiClient.GetAsync("Blanket");
        ...
        public async Task<ActionResult> PurchaseProduct(int id)
        {
            try
            {
                HttpResponseMessage response = await GlobalVariables.WebApiClient.GetAsync("Blanket/" + id.ToString());
        ...
        public async Task<ActionResult> PurchaseProduct(mvcPurchaseModel purchase, int blanketPrice, int blanketId)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        var blanketResponse = await GlobalVariables.WebApiClient.GetAsync("Blanket/" + blanketId.ToString());
        ...
        private async Task<ActionResult> ReloadPurchaseForm(mvcPurchaseModel purchase, int blanketPrice, int blanketId)
        {
            try
            {
                HttpResponseMessage blanketResponse = await GlobalVariables.WebApiClient.GetAsync("Blanket/" + blanketId.ToString());
                if (blanketResponse.IsSuccessStatusCode)
                {
                    var blanket = await blanketResponse.Content.ReadAsAsync<mvcBlanketsModel>();
                    if (blanket != null)
        ...
        private async Task UpdateBlanketQuantity(int blanketId, int purchasedQuantity)
        {
            try
            {
                ...
                HttpResponseMessage response = await GlobalVariables.WebApiClient.GetAsync("Blanket/" + blanketId.ToString());
                if (response.IsSuccessStatusCode)
                {
                    var blanket = await response.Content.ReadAsAsync<mvcBlanketsModel>();
                    if (blanket != null)
                    {
                        ...
                        await GlobalVariables.WebApiClient.PutAsJsonAsync("Blanket/" + blanketId.ToString(), blanket);
                    }
                }
            }
            catch (Exception)
        ...
        {
            IEnumerable<mvcDistributorProducts> blanketList;
            HttpResponseMessage response = GlobalVariables.WebApiClient.GetAsync("DistributorProducts").Result;
            blanketList = response.Content.ReadAsAsync<IEnumerable<mvcDistributorProducts>>().Result;
            return View(blanketList);
        }
        ...
        public ActionResult AddOrEdit(int id = 0)
        {
            if (id == 0)
            {
                return View(new mvcDistributorProducts());
            }
            else
            {
                HttpResponseMessage response = GlobalVariables.WebApiClient.GetAsync("DistributorProducts/" + id.ToString()).Result;
                return View(response.Content.ReadAsAsync<mvcDistributorProducts>().Result);
            }
        }

        [HttpPost]
        public ActionResult AddOrEdit(mvcDistributorProducts sup)
        {
            // Set the supplier's username from the session

            if (sup.BlanketId == 0)
            {
                HttpResponseMessage response = GlobalVariables.WebApiClient.PostAsJsonAsync("DistributorProducts", sup).Result;
                TempData["SuccessMessage"] = "Saved Successfully";
            }
            else
            {
                HttpResponseMessage response = GlobalVariables.WebApiClient.PutAsJsonAsync("DistributorProducts/" + sup.BlanketId, sup).Result;
        ...
        {
            HttpResponseMessage response = GlobalVariables.WebApiClient.DeleteAsync("DistributorProducts/" + id.ToString()).Result;
